    ...
    <head>
        ...
        <script>
            function changeVideo() {
                document.getElementById('bgVideo').querySelector('source').setAttribute('src', 'static/videos/search.mp4');
...
def get_response():
    ...
    <head>
        ...
        <style>
            body {{ font-family: Arial, sans-serif; background: black; color: white; text-align: center; padding: 50px; }}
            .response-container {{
                position: absolute;
                top: 10%;
                left: 50%;
            ...
            a {{ display: block; margin-top: 20px; color: #ff6600; font-size: 1.2rem; text-decoration: none; }}
        </style>
        <script>
            function showResponseText() {{
                ...
                function typeWriter() {{
                    if (i < words.length) {{
                        document.getElementById("responseText").innerHTML += words[i] + " ";
                        i++;
                        setTimeout(typeWriter, 300);  // Adjust speed here (300ms per word)
                    }}
                }}
                typeWriter();
            }}
